...
const OrdersTable = () => {
  ...
  const fetchOrderDetailData = async (orderId) => {
    try {
      const response = await axios.get(
        `http://localhost:9094/api/order/${orderId}`
      );
  ...
  const fetchOrderStatusData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:9094/api/order_status/"
      );
  ...
  const updateOrderStatus = async (orderId, statusId) => {
    try {
      await axios.put(`http://localhost:9094/api/order/${orderId}/${statusId}`);
      fetchOrderData();
    ...
    >
      <div className="flex justify-between items-center mb-6">
        ...
        <dialog id="order_detail_modal" className="modal">
          <div className="modal-box relative w-full max-w-2xl p-6 overflow-y-auto">
            <form method="dialog">
              <button className="btn btn-sm font-bold btn-circle btn-ghost absolute right-2 top-2 bg-red-400 hover:bg-red-500">
                ✕
              </button>
            ...
            {selectedOrder && (
              <>
                ...
                <div className="mb-5">
                  <h4 className="font-semibold mb-3.5">Cập nhật trạng thái:</h4>
